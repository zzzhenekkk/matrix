...
START_TEST(sub_matrix2) {
  ...
  matrix_t res = {0};
  ck_assert_int_eq(s21_sub_matrix(&m, &mtx, &res), INCORRECT_M);
...
TCase *tests_s21_sub_matrix(void) {
  TCase *tcase = tcase_create("case_sub_matrix");

  tcase_add_loop_test(tcase, sub_matrix, 0, 5);
  tcase_add_loop_test(tcase, sub_matrix1, 0, 5);
  tcase_add_loop_test(tcase, sub_matrix2, 0, 5);

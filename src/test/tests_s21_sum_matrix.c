...
START_TEST(sum_matrix2) {
  ...
  ck_assert_int_eq(s21_sum_matrix(&m, &mtx, &res), INCORRECT_M);
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&res);
  s21_remove_matrix(&check);
...
TCase *tests_s21_sum_matrix(void) {
  TCase *tcase = tcase_create("case_sum_matrix");

  tcase_add_loop_test(tcase, sum_matrix, 0, 5);
  tcase_add_loop_test(tcase, sum_matrix1, 0, 5);
  tcase_add_loop_test(tcase, sum_matrix2, 0, 5);
